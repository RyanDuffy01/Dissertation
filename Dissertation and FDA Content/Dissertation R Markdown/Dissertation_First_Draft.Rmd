...
Returning to the example with which the analytical form of the function to estimate is known, the data from this example is non-periodic, e.g. it does not have any repeating patterns, so B-splines can be to estimate the function. 
...
In the example where $f_1(t)$ is being estimated from simulated data stored in the data frame `ex_1_data`, $\boldsymbol{\phi}$  can be found using the eval.basis command from the `fda` package. The first argument passed to this function is the time points which the user wishes to have their basis evaluated at and the second argument supplied is the basis functions, created using the create.fourier.basis or create.bspline.basis within this dissertation. The call to this function which gives the $\boldsymbol{\phi}$ matrix for this example is given below. 
...
The generalized cross-validation (GCV) criteria was developed by Craven P. and G. Wahba (@GCV_ref) to assist in finding the most appropriate value of $\lambda$ to use in smoothing using a roughness penalty. The need for this is clear as choosing a $\lambda$ close to $0$ means that $F(\boldsymbol{c})$ becomes the SSE used to find the coefficient for regression smoothing without a roughness penalty however if the $\lambda$ chosen is too large the estimated function will approach a straight line.  \par 
...
where the SSE is defined as in the [Regression Smoothing] section, the degrees of freedom for a particular functional data object are defined as the trace of the matrix $\boldsymbol{H}$ which is defined in the next paragraph and $n$ is the number of observations in the non-functional data used to estimate the function. \par
